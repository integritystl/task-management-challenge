 ...
 */
const TaskLabel = ({ label }: TaskLabelProps): JSX.Element => {
  return (
    <div
  ...
  return (
    <Card className="group transition-all duration-300 hover:shadow-lg border border-slate-200 hover:border-slate-300 overflow-hidden">
      <div className={cn(
        ...
        task.status === TaskStatus.DONE ? "bg-emerald-400" :
          task.priority === TaskPriority.HIGH ? "bg-rose-400" :
            task.priority === TaskPriority.MEDIUM ? "bg-amber-400" : "bg-blue-400"
      )} />
      <CardHeader className="pb-3 pt-4">
        <div className="flex justify-between items-start gap-4">
          <CardTitle className="text-xl font-semibold text-slate-800 leading-tight">{task.title}</CardTitle>
          <Badge
            variant="outline"
            className={cn(
      ...
      <CardContent className="pb-5">
        {task.description ? (
          <p className="text-slate-600 mb-5 leading-relaxed">{task.description}</p>
        ) : (
        ...
        <CardFooter className="pt-0 pb-4 flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          {onEdit && (
            <Button
              variant="outline"
              size="sm"
              className="rounded-full border-slate-200 text-slate-600 hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50"
              onClick={() => onEdit(task)}
            >
            ...
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  className="rounded-full border-slate-200 text-slate-600 hover:text-rose-600 hover:border-rose-200 hover:bg-rose-50"
                >
              ...
              <AlertDialogContent className="rounded-lg border-slate-200">
                ...
                <AlertDialogFooter>
                  <AlertDialogCancel className="rounded-full border-slate-200 text-slate-700 hover:bg-slate-100 hover:text-slate-900">
                    Cancel
                  </AlertDialogCancel>
                  <AlertDialogAction
