 ...
 */
export async function GET(): Promise<NextResponse<Label[] | { error: string; message: string; type?: string; }>> {
  try {
 ...
 */
export async function POST(request: Request): Promise<NextResponse<Label | { error: string; message: string; details?: Record<string, unknown>; type?: string; }>> {
  try {
 ...
 */
export async function PATCH(request: Request): Promise<NextResponse<Label | { error: string; message: string; details?: Record<string, unknown>; type?: string; }>> {
  try {
    const data = await request.json();
    ...
    if (validData.name !== existingLabel.name) {
      ...
      if (duplicateLabel) {
        return NextResponse.json(
          { error: 'Label name already exists', message: 'Another label with this name already exists' },
          { status: 409 }
        );
      }
    }
 ...
 */
export async function DELETE(request: Request): Promise<NextResponse<{ success: boolean; message: string; } | { error: string; message: string; type?: string; }>> {
  try {
